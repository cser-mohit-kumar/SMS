...
export const AuthProvider = ({ children }) => {
  ...
  useEffect(() => {
    try {
      ...
      if (token && username && role) {
        setUser({ token, refreshToken, username, role });
      }
    } catch (err) {
  ...
  const login = useCallback((token, username, role, refreshToken) => {
    ...
    setUser({ token, refreshToken, username, role: normalizedRole });
  }, []);
